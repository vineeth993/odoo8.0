...
class ir_mail_server(osv.osv):
    ...
    def send_email(self, cr, uid, message, mail_server_id=None, smtp_server=None, smtp_port=None,
        ...
        if mail_server_id:
            ...
            # Added by Vinod
            if message['From']:
               mail_server_ids = self.search(cr, SUPERUSER_ID, [('smtp_user', '=', message['From'])], limit=1)
               if not mail_server_ids:
                   mail_server_ids = self.search(cr, SUPERUSER_ID, [], order='sequence', limit=1)
            # End of Addition
            #mail_server_ids = self.search(cr, SUPERUSER_ID, [], order='sequence', limit=1) #Uncomment this line if removing addition above
            if mail_server_ids:
