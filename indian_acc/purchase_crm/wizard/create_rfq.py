    ...
    def convert_to_rfq(self, cr, uid, ids, context=None):
        ...
        for opportunity in self.browse(cr, uid, ids, context=context):
            ...
            vals = {
                ...
                'location_id': opportunity.warehouse_id.id,
            ...
            for product_id in opportunity.product_ids:
                ...
                res = self.pool.get('purchase.order.line').onchange_product_id(cr, uid, [], pricelist, product.id, 1, product.uom_id.id, partner.id)
                res = res['value']
                line_vals = {
                    'name': res['name'],
